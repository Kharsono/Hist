...
def InverseCompositionAffine(It, It1, threshold, num_iters):
    ...
    # PRE-COMPUTE
    #3. Evaluate gradient of template T
    der_y, der_x = np.gradient(It1)
    delI = np.vstack((np.ravel(der_x), np.ravel(der_y)))
    delI = np.transpose(delI)
    
    #4. Evaluate Jac at x;0 & 5. Compute steepest descent
    ...
    rect_temp = spln_temp.ev(r_coord, c_coord)
