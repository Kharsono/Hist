...
for itr in range(max_iters):
    ...
    for xb,yb in batches:
        ...
        # backward
        diff = pred - yb
        diff = backwards(diff, params, "output", linear_deriv)
        diff = backwards(diff, params, "layer1", sigmoid_deriv)
...
# Training loop before training, just the first loop
xb, yb = batches[0]
# forward
sig = forward(xb, params, "layer1", sigmoid)
pred = forward(sig, params, "output", softmax)
    ...
    if "W" in k:
        ...
        for i in range(v.shape[0]):
            for j in range(v.shape[1]):
                ...
                #   add epsilon
                temp = copy.deepcopy(params_orig)
                temp[k][i, j] += eps
                #   run the network
                sig = forward(xb, temp, "layer1", sigmoid)
                pred = forward(sig, temp, "output", softmax)
                #   get the loss
                loss1, _ = compute_loss_and_acc(yb, pred)
                ...
                pred = forward(sig, temp, "output", softmax)
                #   get the loss
                loss2, _ = compute_loss_and_acc(yb, pred)
